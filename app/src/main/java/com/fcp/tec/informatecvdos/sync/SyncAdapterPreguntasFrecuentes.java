...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloPreguntasFrecuentes;
...
public class SyncAdapterPreguntasFrecuentes extends AbstractThreadedSyncAdapter
{

    public SyncAdapterPreguntasFrecuentes(Context context, boolean autoInitialize)
    {
    ...
    private final String TAG = SyncAdapterPreguntasFrecuentes.class.getSimpleName();
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_PREGUNTAS FRECUENTES: " + UURLS.URL_PREGUNTA_FRECUENTE);

        //Al iniciar la sincronizacion manual o automática se procede a leer la respuesta
        //GET o POST enviada por el servidor desde la URL_PREGUNTA_FRECUENTE dada
        lecturaGET();
    ...
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_PREGUNTA_FRECUENTE, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloPreguntasFrecuentes jsonAModeloPreguntasFrecuentes = new JsonAModeloPreguntasFrecuentes();
            //Se procesa la respuesta JSON y se convierte a objeto Carreras.java
            jsonAModeloPreguntasFrecuentes.procesar(response.getJSONArray("preguntasfrecuentes"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloPreguntasFrecuentes.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                Log.i(TAG, "Cambios en \'preguntas frecuentes\': " + cpOperaciones.size() + " operaciones");


                //UToasts.showToast(getContext(), "Se encontraron " + cpOperaciones.size() + " cambios para sincronizar");
                //APLICAR batch de operaciones
                cResolver.applyBatch(ContractPreguntasFrecuentes.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractPreguntasFrecuentes.URI_CONTENIDO_BASE, null, false);
