...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloLogros;
...
public class SyncAdapterLogros extends AbstractThreadedSyncAdapter
{
    public SyncAdapterLogros(Context context, boolean autoInitialize)
    {
    ...
    private final String TAG = SyncAdapterLogros.class.getSimpleName();
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_LOGROS: " + UURLS.URL_LOGROS);

        //Al iniciar la sincronizacion manual o automática se procede a leer la respuesta
        //GET o POST enviada por el servidor desde la URL_LOGROS dada
        lecturaGET();

    }

    /**
     * Este metodo envia una <b>petición URL_LOGROS</b> para despues leer y almacenar un <B>Objeto de tipo JSON</B>
     * y lo almacena en la variable "response" para su uso posterior.
     * Son obligatorios en una petición URL_LOGROS 2 @Overrides onResponse() y onErrorResponse()
     */
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_LOGROS, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloLogros jsonAModeloLogros = new JsonAModeloLogros();
            //Se procesa la respuesta JSON y se convierte a objeto Carreras.java
            jsonAModeloLogros.procesar(response.getJSONArray("logros"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloLogros.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                Log.i(TAG, "Cambios en \'carreras\': " + cpOperaciones.size() + " operaciones");


                //UToasts.showToast(getContext(), "Se encontraron " + cpOperaciones.size() + " cambios para sincronizar");
                //APLICAR batch de operaciones
                cResolver.applyBatch(ContractLogros.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractLogros.URI_CONTENIDO_BASE, null, false);
