...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloInstalaciones;
...
public class SyncAdapterInstalaciones extends AbstractThreadedSyncAdapter
{
    private final String TAG = SyncAdapterInstalaciones.class.getSimpleName();
    private final ContentResolver cResolver;

    public SyncAdapterInstalaciones(Context context, boolean autoInitialize)
    {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_INSTALACIONES: " + UURLS.URL_INSTALACIONES);

        //Al iniciar la sincronizacion manual o automática se procede a leer la respuesta
        //GET o POST enviada por el servidor desde la URL_INSTALACIONES dada
        lecturaGET();
    ...
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_INSTALACIONES, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloInstalaciones jsonAModeloInstalaciones = new JsonAModeloInstalaciones();
            //Se procesa la respuesta JSON y se convierte a objeto Carreras.java
            jsonAModeloInstalaciones.procesar(response.getJSONArray("instalaciones"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloInstalaciones.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                cResolver.applyBatch(ContractInstalaciones.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractInstalaciones.URI_CONTENIDO_BASE, null, false);
