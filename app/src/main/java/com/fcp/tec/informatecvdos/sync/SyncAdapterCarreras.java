...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloCarreras;
...
public class SyncAdapterCarreras extends AbstractThreadedSyncAdapter
{
    public SyncAdapterCarreras(Context context, boolean autoInitialize)
    {
    ...
    private final String TAG = SyncAdapterCarreras.class.getSimpleName();
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_CARRERAS: " + UURLS.URL_CARRERA);

        //Al iniciar la sincronizacion manual o automática se procede a leer la respuesta
        //GET o POST enviada por el servidor desde la URL_CARRERA dada
        lecturaGET();
    ...
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_CARRERA, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloCarreras jsonAModeloCarreras = new JsonAModeloCarreras();
            //Se procesa la respuesta JSON y se convierte a objeto Carreras.java
            jsonAModeloCarreras.procesar(response.getJSONArray("carreras"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloCarreras.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                cResolver.applyBatch(ContractCarreras.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractCarreras.URI_CONTENIDO_BASE, null, false);
