...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloPreguntasTest;
...
public class SyncAdapterPreguntasTest extends AbstractThreadedSyncAdapter
{

    private final String TAG = SyncAdapterPreguntasTest.class.getSimpleName();
    private final ContentResolver cResolver;


    public SyncAdapterPreguntasTest(Context context, boolean autoInitialize)
    {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_PREGUNTA: " + UURLS.URL_PREGUNTA);
    ...
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_PREGUNTA, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloPreguntasTest jsonAModeloPreguntasTest = new JsonAModeloPreguntasTest();
            //Se procesa la respuesta JSON y se convierte a objeto Beca.java
            jsonAModeloPreguntasTest.procesar(response.getJSONArray("preguntastest"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloPreguntasTest.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                Log.i(TAG, "Cambios en \'preguntas\': " + cpOperaciones.size() + " operaciones");


                //UToasts.showToast(getContext(), "Se encontraron " + cpOperaciones.size() + " cambios para sincronizar");
                //APLICAR batch de operaciones
                cResolver.applyBatch(ContractPreguntasTest.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractPreguntasTest.URI_CONTENIDO_BASE, null, false);
