...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloReqInscripcion;
...
public class SyncAdapterReqInscripcion extends AbstractThreadedSyncAdapter
{

    private final String TAG = SyncAdapterReqInscripcion.class.getSimpleName();
    private final ContentResolver cResolver;


    public SyncAdapterReqInscripcion(Context context, boolean autoInitialize)
    {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_REQUISITOS_INSCRIPCION: " + UURLS.URL_REQUISITOS_INSCRIPCION);

        //Al iniciar la sincronizacion manual o automática se procede a leer la respuesta
        //GET o POST enviada por el servidor desde la URL_REQUISITOS_INSCRIPCION dada
        lecturaGET();

    }

    /**
     * Este metodo envia una <b>petición URL_REQUISITOS_INSCRIPCION</b> para despues leer y almacenar un <B>Objeto de tipo JSON</B>
     * y lo almacena en la variable "response" para su uso posterior.
     * Son obligatorios en una petición URL_REQUISITOS_INSCRIPCION 2 @Overrides onResponse() y onErrorResponse()
     */
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_REQUISITOS_INSCRIPCION, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    /**
     * este método procesa el json que acaba de obtenerse en la petición URL_REQUISITOS_INSCRIPCION
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloReqInscripcion jsonAModeloReqInscripcion = new JsonAModeloReqInscripcion();
            //Se procesa la respuesta JSON y se convierte a objeto Beca.java
            jsonAModeloReqInscripcion.procesar(response.getJSONArray("requisitoInscripcion"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloReqInscripcion.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                cResolver.applyBatch(ContractReqInscripcion.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractReqInscripcion.URI_CONTENIDO_BASE, null, false);
