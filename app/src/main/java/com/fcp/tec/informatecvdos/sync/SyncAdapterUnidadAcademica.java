...
import com.fcp.tec.informatecvdos.modelos.JsonAModeloUnidadAcademica;
...
public class SyncAdapterUnidadAcademica extends AbstractThreadedSyncAdapter
{

    private final String TAG = SyncAdapterUnidadAcademica.class.getSimpleName();
    private final ContentResolver cResolver;


    public SyncAdapterUnidadAcademica(Context context, boolean autoInitialize)
    {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
    {
        ...
        Log.i(TAG, "Sincronizando desde la URL_UNIDAD_ACADEMICA: " + UURLS.URL_UNIDAD_ACADEMICA);
    ...
    private void lecturaGET()
    {
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest
                (Request.Method.GET, UURLS.URL_UNIDAD_ACADEMICA, new Response.Listener<JSONObject>()
                {
                    @Override
    ...
    private void procesarGET(JSONObject response)
    {
        try
        {
            ...
            JsonAModeloUnidadAcademica jsonAModeloUnidadAcademica = new JsonAModeloUnidadAcademica();
            //Se procesa la respuesta JSON y se convierte a objeto Beca.java
            jsonAModeloUnidadAcademica.procesar(response.getJSONArray("unidadacademica"));


            //Se procesar las operaciones correspondienes al CRUD y comparaciones
            jsonAModeloUnidadAcademica.procesarOperaciones(cpOperaciones, cResolver);
            ...
            if (cpOperaciones.size() > 0)
            {
                ...
                cResolver.applyBatch(ContractUnidadAcademica.AUTORIDAD, cpOperaciones);

                //NOTIFICAR cambio al Content Provider
                cResolver.notifyChange(ContractUnidadAcademica.URI_CONTENIDO_BASE, null, false);
